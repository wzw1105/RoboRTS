...
{
  std::vector<ScanBlock> scan_blocks(8);

  int index = 0;
  while (index < 8) {
    error_t result = readScanBlock(scan_blocks[index]);
    if (result != error_t::no_error)
      return result;

    if (scan_blocks[index].block_id != index) {
      index = 0;
      continue;
    }
    else
  ...
  scan_frame.timestamp = scan_blocks[0].timestamp;
  int count = scan_blocks[0].layers[0].ranges.size();
  ...
  for (index = 0; index < 8; index++) {
    for (int i = 0; i < count; i++) {
      scan_frame.layers[0].ranges[index * count + i] = scan_blocks[index].layers[0].ranges[i];
      scan_frame.layers[0].intensities[index * count + i] = scan_blocks[index].layers[0].intensities[i];
    }
  }
