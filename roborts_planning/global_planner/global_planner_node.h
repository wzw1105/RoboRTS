...
class GlobalPlannerNode {
 ...
 private:
  ...
  std::vector<geometry_msgs::PoseStamped> FloydSmooth( std::vector<geometry_msgs::PoseStamped> path);
  bool CheckCross(geometry_msgs::PoseStamped start,geometry_msgs::PoseStamped end);
  double GetNextY(double x, double y, double k);
  void AddPoint(std::vector<geometry_msgs::PoseStamped> &path, geometry_msgs::PoseStamped &start, geometry_msgs::PoseStamped &end);

  /**
   * @brief check the cost of the cells int a vertical line whether bigger than 252
   * 
   */
  bool CheckVerticalLineCost(int x, int y_1, int y_2); 
