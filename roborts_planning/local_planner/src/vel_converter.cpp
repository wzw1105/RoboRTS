...
class VelConverter {
 public:
  VelConverter() : new_cmd_acc_(false), begin_(false) {
    ...
    // local_planner_state_sub_ = cmd_handle_.subscribe<std_msgs::Bool>("/local_planner_state", 2,boost::bind(&VelConverter::LocalPlannerStateCallback_, this, _1));
  }
  void VelCallback(const roborts_msgs::TwistAccel::ConstPtr& msg);
  // void LocalPlannerStateCallback_(const std_msgs::Bool::ConstPtr &state);
  void UpdateVel();
...
private:
  ...
  //bool vel_converter_active;

  ros::NodeHandle cmd_handle_;
  ros::Publisher cmd_pub_;
  ros::Subscriber cmd_sub_;
  //* 订阅local planner的执行状态
...
// void VelConverter::LocalPlannerStateCallback_(const std_msgs::Bool::ConstPtr &state) 
// {
//   this->vel_converter_active = state->data;
// }
