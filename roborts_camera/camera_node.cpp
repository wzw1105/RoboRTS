...
CameraNode::CameraNode() {
  ...
  for (unsigned int i = 0; i < camera_num_; i++) {
    ...
    camera_driver_[i] = roborts_common::AlgorithmFactory<
        CameraBase, CameraInfo>::CreateAlgorithm(camera_info.camera_type,
                                                 camera_info);
  }
  StartThread();
}
  ...
  while (running_) {
    ...
    if (!img.empty()) {
      ...
      img_msg->header.frame_id =
          camera_param_.GetCameraParam()[index].camera_name;
      img_msg->header.stamp = ros::Time::now();

      camera_param_.GetCameraParam()[index].ros_camera_info->header.stamp =
          img_msg->header.stamp;
      img_pubs_[index].publish(
          img_msg, camera_param_.GetCameraParam()[index].ros_camera_info);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "roborts_camera_node",
