#ifndef MOVE_TO_BLOOD_ADD_AREA_H
#define MOVE_TO_BLOOD_ADD_AREA_H
...
class MoveToBloodAddArea : public BT::CoroActionNode {
public:
	MoveToBloodAddArea(const std::string& name) : CoroActionNode(name,{}) {
    ...
    BT::NodeStatus tick() override{
        ...
        geometry_msgs::PoseStamped blood_pos = this -> blackboard_ptr_ -> F_1_6[this -> blackboard_ptr_ -> blood_add_position_id];
        std::cout << "Move To Blood Add Area: " <<  blood_pos.pose.position.x << " "  << blood_pos.pose.position.y << "\n";
        this -> chassis_executor_ -> Execute(blood_pos);
