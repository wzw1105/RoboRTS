...
class ChassisExecutor{
 ...
 private:
  ...
  //the modified yaw value of start position and destination position
  double init_yaw, last_yaw;

  //store the path calculated by global planner
  nav_msgs::Path global_path;

  void poseCallBack(const nav_msgs::Odometry &odom);
  double T_angle(double a);
  //rotate a specific angle by the rotation velocity
  void RotateByTime(double angle);
  //rotate the direction of the robot to a specific point
  double RotateToPoint(tf::Point point);
  double DeterminDirection(geometry_msgs::PoseStamped a, geometry_msgs::PoseStamped b);
  // ~add
