...
ChassisExecutor::ChassisExecutor():execution_mode_(ENUM_CLASS::ChassisExcutionMode::IDLE_MODE), execution_state_(ENUM_CLASS::BehaviorState::IDLE),
                                   global_planner_client_("global_planner_node_action", true), //! true -> don't need ros::spin()
                                   local_planner_client_("local_planner_node_action", true)
{
  ...
  this->execution_mode_ = ChassisExcutionMode::IDLE_MODE;
}

void ChassisExecutor::Execute(const geometry_msgs::PoseStamped &goal){
  execution_mode_ = ChassisExcutionMode::GOAL_MODE;
...
void ChassisExecutor::Execute(const geometry_msgs::Twist &twist){
  if( execution_mode_ == ChassisExcutionMode::GOAL_MODE){
    Cancel();
  }
  execution_mode_ = ChassisExcutionMode::SPEED_MODE;
...
void ChassisExecutor::Execute(const roborts_msgs::TwistAccel &twist_accel){
  ...
  execution_mode_ = ChassisExcutionMode::SPEED_WITH_ACCEL_MODE;
...
BehaviorState ChassisExecutor::Update(){
  ...
  switch (execution_mode_){
    case ChassisExcutionMode::IDLE_MODE:
      execution_state_ = BehaviorState::IDLE;
      break;

    case ChassisExcutionMode::GOAL_MODE:
    ...
    case ChassisExcutionMode::SPEED_MODE:
      execution_state_ = BehaviorState::RUNNING;
      break;

    case ChassisExcutionMode::SPEED_WITH_ACCEL_MODE:
      execution_state_ = BehaviorState::RUNNING;
      break;
...
void ChassisExecutor::Cancel(){
  switch (execution_mode_){
    ...
    case ChassisExcutionMode::GOAL_MODE:
      ...
      execution_mode_ = ChassisExcutionMode::IDLE_MODE;
      break;
