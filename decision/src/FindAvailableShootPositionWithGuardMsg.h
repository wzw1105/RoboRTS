...
public:
    ...
    BT::NodeStatus tick() override{
        ...
        if( !ChooseOptimalGoal(blackboard_ptr_ ->guard_cars_msg_, result) ) {
...
private:
    ...
    bool GetAreaIndex(float x, float y, int &x_index, int &y_index) { //get integer index of pos(x , y), stored in (x_index, y_index)
        ...
        ROS_INFO("GetAreaIndex: x = %.1f, y = %.1f, x_index = %d, y_index = %d", x, y, x_index, y_index);
        return true;
    }
    ...
    bool CalculateAreaCenter(int x_index, int y_index, geometry_msgs::PoseStamped &shoot_pose) {
        ...
        ROS_INFO("CalculateAreaCenter: x_index = %d, y_index = %d, centerx = %.1lf, centery = %.1lf", x_index, y_index, shoot_pose.pose.position.x, shoot_pose.pose.position.y);
    ...
    bool ChooseOptimalGoal(roborts_msgs::CarMsgs all_cars, roborts_msgs::CarMsg &choosed_goal) {
        ...
        //Normal Status
        if(blackboard_ptr_ ->lurk_mode != LurkingStatus::Lurking) {
    ...
    float EvaluateCarScore(roborts_msgs::CarMsg &car) {
        float self_x = blackboard_ptr_ -> my_Pose.pose.position.x, self_y = blackboard_ptr_ -> my_Pose.pose.position.y;
