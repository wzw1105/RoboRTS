...
class LocalizationNode {
 ...
 private:
  ...
  int remaining_time_;
  int fusion_time_start_;
  int fusion_time_end_;
  //Mutex
  std::mutex mutex_;
  ...
  //* 订阅裁判系统发布的ID
  ros::Subscriber referee_robot_id_sub_;
...
//  bool enable_uwb_;
  ...
  //添加add pose information from post
  ...
  //添加哨岗位置订阅
  ros::Subscriber pose_from_post_sub;

  geometry_msgs::PoseStamped pose_from_post;

  void PosefromPost(const roborts_msgs::CarMsgs::ConstPtr &pose_from_post_msg);

  void CallBack_Sub_Referee_ID(const roborts_msgs::RobotStatus::ConstPtr& robot_status);
  void CallBack_Sub_Referee_Game_Status(const roborts_msgs::GameStatus::ConstPtr& game_status);
