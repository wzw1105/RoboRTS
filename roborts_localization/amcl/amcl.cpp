...
void Amcl::Init(const Vec3d &init_pose, const Vec3d &init_cov) {
  ...
  resample_interval_=amcl_param_.resample_interval;
...
int Amcl::Update(const Vec3d &pose,
  ...
  //std::cout<<"AMCL is updating!"<<std::this_thread::get_id()<<std::endl;
  ...
  UpdateOdomPoseData(pose,received_);

  resampled_ = false;
  if (laser_update_) {
    UpdateLaser(laser_scan, angle_min, angle_increment, pose, particle_cloud_pose_msg,received_,pose_from_post_);
  }
  UpdateFilter(hyp_pose, laser_scan.header.stamp);
};

void Amcl::UpdateOdomPoseData(Vec3d pose, bool pose_received_) {
  Vec3d delta;
  delta.setZero();
...
void Amcl::UpdateLaser(const sensor_msgs::LaserScan &laser_scan,
  ...
  particle_cloud_pose_msg = ResampleParticles(received_,pose_from_post_);

}

geometry_msgs::PoseArray Amcl::ResampleParticles(bool received_,// 添加
                 geometry_msgs::PoseStamped pose_from_post_ //添加add pose
                 ) {
  ...
  if (!(++resample_count_ % resample_interval_)) {
    if(resample_count_ > 1 ){
      ...
      pf_ptr_->UpdateResample(received_,pose_from_post_);
      resampled_ = true;
