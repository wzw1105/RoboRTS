...
void ParticleFilter::UpdateResample(bool received_,// 添加
                 geometry_msgs::PoseStamped pose_from_post_ //添加add pose
                 ) {
  ...
  //ROS_INFO("filter msgs:%d %f %f %f",received_ ? 1: 0, pose_from_post_.pose.position.x, pose_from_post_.pose.position.y, pose_from_post_.pose.position.z);
 
  if (received_)
    std::cout << "filter received pose! " << pose_from_post_.pose.position.x 
     << " " <<pose_from_post_.pose.position.y << "  "<<std::this_thread::get_id()<<std::endl;

  //添加 
  Vec3d post_pose;
  Mat3d post_pose_cov;
  post_pose_cov(0,0)=0.5*0.5;  
  post_pose_cov(0,1)=0.0; 
  post_pose_cov(0,2)=0.0; 
  post_pose_cov(1,0)=0.0; 
  post_pose_cov(1,1)=0.5*0.5; 
  post_pose_cov(1,2)=0.0; 
  post_pose_cov(2,0)=0.0; 
  post_pose_cov(2,1)=0.0; 
  post_pose_cov(2,2)=(M_PI/12.0)*(M_PI/12.0); 
  post_pose[0]=pose_from_post_.pose.position.x;
  post_pose[1]=pose_from_post_.pose.position.y;
  post_pose[2]=pose_from_post_.pose.position.z;
/**
 * @brief 
 * if current time - pose_from_post_.header.sec < 1s && 
 *   then useful
 */

  auto pf_gaussian_pdf = ParticleFilterGaussianPdf(post_pose, post_pose_cov);

  while (set_b->sample_count < this->max_samples_) {
    sample_b = &set_b->samples_vec[set_b->sample_count++];
    //判断是否添加哨岗信息
    if(received_){
      sample_b->pose = pf_gaussian_pdf.GenerateSample();
      sample_b->pose[2]=drand48()*2*M_PI-M_PI;
    }
    else{
      if (drand48() < w_diff){
